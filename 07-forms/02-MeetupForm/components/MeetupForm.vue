    ...
    <div class="meetup-form__content">
      <fieldset class="meetup-form__section">
        <ui-form-group label="Название">
          <ui-input name="title" v-model="meetupItem.title" />
        </ui-form-group>
        <ui-form-group label="Дата">
          <ui-input-date type="date" name="date" v-model="meetupItem.date" />
        </ui-form-group>
        <ui-form-group label="Место">
          <ui-input name="place" v-model="meetupItem.place" />
        </ui-form-group>
        <ui-form-group label="Описание">
          <ui-input multiline rows="3" name="description" v-model="meetupItem.description" />
        </ui-form-group>
        <ui-form-group label="Изображение">
          ...
          <ui-image-uploader
            name="image"
            :preview="meetupItem.image"
            @select="meetupItem.imageToUpload = $event"
            @remove="meetupItem.imageToUpload = null"
          />
        </ui-form-group>
      </fieldset>
...
import { createMeetup, createAgendaItem } from '../meetupService';
...
export default {
  ...
  watch: {
    meetup: {
      immediate: true,
      handler(newMeetup) {
        if (!isEqual(newMeetup, this.meetupItem)) {
          this.meetupItem = cloneDeep(newMeetup);
          this.meetupItem.agenda = cloneDeep(newMeetup.agenda);
        }
      },
    },
  },
  ...
  methods: {
    addAgendaItem() {
      const agenda = this.meetupItem.agenda;

      const newItem = createAgendaItem();
      if (agenda.length) {
        newItem.startsAt = agenda[agenda.length - 1].endsAt;
      }
      agenda.push(newItem);
    },
    ...
    submitHandler() {
      this.$emit('submit', cloneDeep(this.meetupItem));
    },
  },
};
</script>
